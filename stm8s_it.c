...
/**
 ...
 INTERRUPT_HANDLER(TIM4_UPD_OVF_IRQHandler, 23)
 {
  /* In order to detect unexpected events during development,
     it is recommended to set a breakpoint on the following instruction.
  */
	/*I2C_SendData(counter);  // Send data via SPI
		++counter;
		//printf("Current counter is");
		if (counter == 4) {
				counter = 0;  // Reset the counter if it reaches 256 (8 bits)
		}

	TIM4_ClearITPendingBit(TIM4_IT_UPDATE); // Clear the Timer1 interrupt flag
*/
